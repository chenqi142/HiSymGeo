...
class YOLOLayer(nn.Module):
    ...
    def forward(self, x, targets=None):
        ...
        grid_x = grid_x.to(x.data.device)
        grid_y = grid_y.to(y.data.device)
        anchor_w = anchor_w.to(w.data.device)
        anchor_h = anchor_h.to(h.data.device)
        ...
        if targets is not None:
            ...
            pred_boxes = pred_boxes.to(pred_cls.device)
            output = torch.cat(
                (
